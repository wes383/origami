export const zh = {
  docTitle: "Origami",
  fullscreen: "全屏",
  exitFullscreen: "退出全屏",
  pinToolbar: "固定工具栏",
  unpinToolbar: "取消固定",
  shortcutFullscreen: "切换全屏",
  openDropHint: "或将 PDF 文件拖拽到此处打开",
  openPdf: "打开 PDF",
  prevPage: "上一页",
  nextPage: "下一页",
  singlePage: "单页",
  continuous: "滚动",
  doublePage: "双页",
  pageLayout: "页面布局",
  rotatePage: "旋转页面",
  print: "打印",
  fileProperties: "文件属性",
  moreActions: "更多操作",
  preparingPrint: "正在准备打印…",
  printAll: "全部页",
  printCurrent: "当前页",
  printCustom: "自定义范围",
  printRangeHint: "页码范围（如 1-3, 5, 8-10）",
  printRangeInvalid: "请输入有效的页码范围",
  printPageCount: "共 {n} 页",
  flipMode: "翻页模式",
  paged: "翻页",
  zoomIn: "放大",
  zoomOut: "缩小",
  zoom: "缩放",
  fitWidth: "适应宽度",
  fitPage: "适合页面",
  toggleTheme: "切换深浅主题",
  theme: "主题",
  lightTheme: "浅色",
  darkTheme: "深色",
  systemTheme: "系统",
  // 页面滤镜
  pageFilter: "页面滤镜",
  filterOff: "关闭",
  filterNight: "夜间",
  filterPaper: "纸色",
  filterCustom: "自定义",
  filterBrightness: "亮度",
  filterContrast: "对比度",
  filterWarmth: "色温",
  closeFile: "关闭文件",
  settings: "设置",
  language: "语言",
  pageJump: "页码",
  loading: "正在加载…",
  errorNotPdf: "仅支持打开 PDF 文件",
  errorInvalid: "无法打开该文件，可能已损坏或不是有效的 PDF",
  renderError: "页面渲染出错",
  retry: "重试",
  dropToOpen: "松开以打开文件",
  recent: "最近打开",
  removeFromRecent: "从最近打开中移除",
  showInFolder: "打开所在文件夹",
  // 目录 / 缩略图
  toc: "目录",
  tocClose: "关闭目录",
  thumbnails: "页面缩略图",
  // 全文查找
  search: "查找",
  searchPlaceholder: "查找文本…",
  searchPrev: "上一个结果",
  searchNext: "下一个结果",
  searchClose: "关闭查找",
  // AI 划词/划句翻译
  aiSettings: "AI 翻译设置",
  aiTranslate: "AI 翻译",
  aiModeWord: "词语释义",
  aiModeSentence: "整句翻译",
  aiInContext: "本文含义",
  aiSenses: "常见含义",
  aiLoading: "正在分析…",
  aiRetry: "重试",
  aiClose: "关闭",
  aiSummarize: "AI 总结",
  aiErrorRequest: "请求失败",
  aiErrorNetwork: "网络请求失败，请检查网络与接口地址",
  aiErrorAuth: "认证失败，请检查 API Key",
  aiErrorNotFound: "接口地址不存在，请检查 Base URL 与模型名",
  aiErrorRateLimit: "请求过于频繁或额度不足",
  aiErrorHttp: "接口返回错误",
  aiErrorEmpty: "AI 未返回内容",
  aiBaseUrl: "接口地址 (Base URL)",
  aiApiKey: "API Key",
  aiSavedModels: "已保存的模型（点击选用）",
  aiBaseUrlPlaceholder: "https://api.openai.com/v1",
  aiShow: "显示",
  aiHide: "隐藏",
  aiAddTitle: "添加模型",
  aiEditTitle: "编辑模型",
  aiEdit: "编辑",
  aiSaveEdit: "保存修改",
  aiAddModel: "添加",
  aiName: "显示名（可选）",
  aiModelName: "模型名",
  aiSwitchModel: "切换模型",
  aiTargetLang: "目标语言",
  aiTargetAuto: "跟随界面语言",
  aiSetActive: "设为当前使用",
  aiActive: "当前使用",
  aiRemoveModel: "删除",
  aiNoProfiles: "尚未保存模型，请在下方添加",
  aiErrorNoModel: "未配置模型，请先在设置中添加",
  aiTest: "测试连接",
  aiTesting: "测试中…",
  aiTestOk: "连接成功，配置可用",
  aiTestFail: "连接失败",
  aiConfigHint:
    "兼容 OpenAI Chat Completions 协议（OpenAI / DeepSeek / Kimi / Ollama 等）。配置仅保存在本机，不会上传到任何服务器。",
  aiCancel: "取消",
  aiFillFirst: "请完整填写 Base URL、API Key 与模型名",
  copy: "复制",
  // Wikipedia 名词解释
  wikiLoading: "正在获取 Wikipedia 摘要…",
  wikiNotFound: "未找到相关条目",
  wikiEmpty: "该条目没有可用的摘要内容",
  wikiOpen: "在 Wikipedia 中打开",
  // 加密文档
  pdfPasswordTitle: "需要密码",
  pdfPasswordDesc: "该文档已加密，请输入打开密码",
  pdfPasswordWrong: "密码错误，请重试",
  pdfPasswordPlaceholder: "文档密码",
  pdfPasswordOpen: "打开",
  cancel: "取消",
  // 全文查找
  searchScanning: "已扫描",
searchStop: "停止查找",
searchCaseSensitive: "区分大小写",
searchWholeWord: "全词匹配",
searchRegex: "正则表达式",
searchResultList: "结果列表",
searchTruncated: "结果过多，仅显示前 500 条，请缩小范围",
searchRegexError: "正则无效或会产生零宽匹配",
  clearAllRecent: "清空最近打开",
  errorMultipleFiles: "一次只能打开一个文件",
  // 目录搜索
  tocSearchPlaceholder: "搜索目录…",
  tocMatchCount: "{n} 个结果",
  tocNoMatch: "未找到匹配的目录项",
  // 快捷键
  shortcutsTitle: "快捷键",
  shortcutSidebar: "切换目录侧边栏",
  shortcutTranslateSidebar: "切换翻译侧边栏",
  shortcutZoomReset: "重置缩放（100%）",
  shortcutFit: "切换适应宽度 / 页面",
  shortcutWikipedia: "Wikipedia 搜索",
  shortcutSummary: "AI 总结",
  shortcutHelp: "显示本帮助",
  shortcutSelectionHint: "选中文本后可用",
  aiSidebar: "翻译侧边栏",
  panelEmptyTranslate: "选中 PDF 文本后，在此查看 AI 翻译结果",
  panelEmptyWikipedia: "选中 PDF 文本后，在此查看 Wikipedia 解释",
  panelEmptySummary: "在 PDF 中选中长文本后，在此查看 AI 总结",
  // 文件属性弹窗
  fpFileName: "文件名",
  fpPages: "页数",
  fpSize: "文件大小",
  fpVersion: "PDF 版本",
  fpTitle: "标题",
  fpAuthor: "作者",
  fpSubject: "主题",
  fpKeywords: "关键词",
  fpCreator: "创建者",
  fpProducer: "生成程序",
  fpCreated: "创建时间",
  fpModified: "修改时间",
  fpLoading: "正在读取…",
  fpPath: "文件路径",
  fpEncrypted: "加密",
  fpPureXfa: "纯 XFA 文档",
  fpFingerprint: "文件指纹",
  fpLinearized: "线性化",
  fpTrapped: "陷印标记",
  fpAcroForm: "AcroForm 表单",
  fpXfa: "XFA 表单",
  fpCollection: "文件集合",
  fpSignatures: "数字签名",
  fpAttachments: "附件",
  fpCustom: "自定义属性",
  fpYes: "是",
  fpNo: "否",
  fpUnknown: "未知",
  autoScroll: "自动阅读",
  autoScrollPause: "暂停自动阅读",
  autoScrollSpeed: "阅读速度",
  autoScrollSpace: "自动阅读 播放 / 暂停",
  autoScrollSlower: "减速",
  autoScrollFaster: "加速",
  autoScrollCustom: "自定义速度",
  autoScrollStop: "停止自动阅读",
  autoSecPerPage: "秒/页",
  bookmarks: "书签",
  addBookmark: "添加书签",
  removeBookmark: "移除书签",
  bookmarkEmpty: "暂无书签",
  bookmarkAddCurrent: "添加当前页",
  bookmarkRename: "重命名书签",
  listEdit: "编辑列表",
  listEditDone: "完成",
  bookmarkDefaultLabel: "第 {n} 页",
  annotationListEmpty: "暂无注释",
  annotateHighlight: "高亮",
  annotateUnderline: "下划线",
  annotateStrikeout: "删除线",
  annotateNote: "文字批注",
  annotateDelete: "删除批注",
  annotateNotePlaceholder: "输入批注内容…",
  annotateSave: "保存",
  annotateTools: "注释",
  annotateAll: "全部",
};

export type LangKeys = keyof typeof zh;
